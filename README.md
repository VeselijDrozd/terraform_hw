...
Единственный вариант удаленного управления docker для terraform, который я нашел - указать в параметрах провайдера host. Поскольку terraform общается напрямую с docker API, context описанный в docker CLI не будет работать для terraform. Но в deploy.sh я все равно оставил создание context. Просто для удобства.

---
*P.S. Надеюсь, я нигде не оставил дыр. Пошел смотреть следующую лекцию, пока совсем безнадежно не отстал.*
